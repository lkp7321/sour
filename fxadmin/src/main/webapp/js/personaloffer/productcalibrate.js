...
require(['jquery','mmGrid','niceScroll','wdatePicker','dialog'],function($,mmGrid,niceScroll,wdatePicker,dialog){
    ...
    var cols = [
        ...
        { title:'买入价上限', name:'mxmd' ,width:100, align:'right'},
				...
				success:function(data){
					if(data.code==00){
						...
						if( userdata==1){                                            //判断展示对应的弹出框；
				            ...
				            checkfn({'url':'ChkParaInit.do','data':{userKey:userKey, exnm:exnm },'obj':'productcalibrate','product':product});
						}else if( userdata==0){
							dialog.priceModify("productcalibrate",0);
				             ...
				             checkfn({'url':'MarkInit2.do','data':{userKey:userKey,exnm:exnm },'obj':'productcalibrate','product':product});  //相当于初始化产品市场源头选择；
						}
						//点击钞汇标志；
 ...
 //保存校验；
	...
	$('body',parent.document).on('click','.pricecontmain li',function(){         
        var txt=$(this).text(),
			...
			PriceVo;
		if(txt=='校验'){
			checkfn({'url':'ChkParaInit.do','data':{userKey:userKey, exnm:exnm},'obj':'productcalibrate'});
		}else if(txt=='停牌'){
