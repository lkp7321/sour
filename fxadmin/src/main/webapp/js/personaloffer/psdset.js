require.config({
 ...
 require(['jquery','dialog'],function($,dialog){
     ...
     $('.psdret_save').click(function(){
 		  	 var oldpsd=$('#old').val(),
 			  	 newpsd=$('#new').val(),
 			  	 reppsd=$('#repeat').val(),errmsg;
			   	 var vo={ "loginuser":user,"user":{'pswd':oldpsd},'newpswd':newpsd};
					 ...
					 //请求数据
						$.ajax({
							...
							success:function(data){
								if(data.code==00){
									 dialog.choicedata(data.codeMessage,'psdset');
     ...
     //关闭弹出窗  
	 $('body',parent.document).on('click','.psdset .sure',function(){
