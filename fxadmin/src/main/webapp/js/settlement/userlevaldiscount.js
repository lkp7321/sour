...
require(['jquery','mmGrid','niceScroll','wdatePicker','dialog'],function($,mmGrid,niceScroll,wdatePicker,dialog){
    ...
    var ogcd=sessionStorage.obj,
        level=$('.long option:selected').val(),
        ogcdVo={'level':level,'ogcd':ogcd,'pageNo':1,'pageSize':10} 
    getOrgnlist(ogcdVo);
    renpage();
    //根据客户等级的改变重新请求列表
  ...
  //封装点击选择 请求机构
    ...
    //点击添加按钮；
	$('.add').click(function(){
		dialog.discountAdd('userlevaldiscount','客户等级优惠','保存');
		$('.agentnum',parent.document).val($('.agentnum').val());
		$('.userleval',parent.document).val($('.long option:selected').text());
		$('.userleval',parent.document).attr('level',$('.long option:selected').val());
	...
	$('.modify').click(function(){
		...
		if(a==0){
			dialog.choicedata('请选择一条数据','userlevaldiscount');
		}else if(a==1){
			dialog.discountAdd('userlevaldiscount','客户等级优惠','修改');
			$('.agentnum',parent.document).val( $('.agentnum').val());
			$('.userleval',parent.document).val($('.long option:selected').text());
			$('.userleval',parent.document).attr('level',$('.long option:selected').val());
			...
			dialog.discountAdd('userlevaldiscount','机构优惠修改','修改');
			$('.agentnum',parent.document).val( $('.agentnum').val());
		}
		$('.buy',parent.document).blur(function(){
			 if($(this).val()==''||$(this).val()==undefined){
	...
	$('body',parent.document).on('click','.preserve',function(){
	    var ogcd=$('.agentnum').val(),
	        level=$('.userleval',parent.document).attr('level'),
	        ...
	        vo={'userKey':userkey,'ogcd':ogcd,'level':level,'currency':currency,'byds':byds,'slds':slds,'beginDate':beginDate,'endDate':endDate},arr=[];
	  ...
	  //判断结束日期不能为空
	    ...
	    if($(this).text()=="保存"){
		  if(num>=4){ 
		   $.ajax({
				...
				success:function(data){
					 if(data.code==00){
					     ...
					     getOrgnlist(ogcdVo); 
						 renpage();
					 	 dialog.choicedata(data.codeMessage,'userlevaldiscount');
					 }else if(data.code==01){
						//异常
						 dialog.choicedata(data.codeMessage,'userlevaldiscount');
					}
		   ...
		   vo={'userKey':userkey,'ogcd':ogcd,'level':level,'byds':byds,'slds':slds,'beginDate':beginDate,'endDate':endDate,'chlist':arr};
		  if(num>=4){
		   $.ajax({
				url:'/fx/updateRuleByCustLevels.do',
				type:'post',
				contentType:'application/json',
				data:JSON.stringify(vo),
				async:true,
	...
	$('body',parent.document).on('click','.discountpub .confirm',function(){
		   var ogcd=$('.agentnum').val(),
		       level=$('.long option:selected').val(),arr=[],vo;
		   $('.box tr').each(function(i,v){
				if( $(v).find('input[type="checkbox"]').prop('checked') ){
				  arr.push({
					  'exnm':$(v).find('td').eq(1).find('span').text()
		  ...
		  vo={'userkey':userkey,'ogcd':ogcd,'level':level,'chlist':arr};
		   $.ajax({
				url:'/fx/deleteCustLevelFavRules.do',
				type:'post',
				contentType:'application/json',
				data:JSON.stringify(vo),
				async:true,
				success:function(data){
					 if(data.code==00){
						 ...
						 ogcd=$('.agentnum').val(),
					     ogcdVo={'ogcd':ogcd,'level':level,'pageNo':1,'pageSize':10} 
					     getOrgnlist(ogcdVo); 
						 renpage();
					 	 dialog.choicedata(data.codeMessage,'userlevaldiscount');
					 }else if(data.code==01){
	...
	//点击请选择一条数据的确定按钮；
	$('body',parent.document).on('click','.userlevaldiscount .sure',function(){
		  $(this).closest('.zhezhao').remove();
	});
	//关闭弹窗
	...
	function getOrgnlist(obj){
		$.ajax({
    		url:'/fx/getCustFavRuleByOgcd.do',
    		type:'post',
    		contentType:'application/json',
    		data:JSON.stringify(obj),
    		async:false,
    ...
    $('.boxtop').on('click','.page .first',function(){
    	var Nopage=parseInt($('.Nopage').text());
 	    if(Nopage>1){
 	    	...
 	    	ogcd=$('.agentnum').val(),
 	        level=$('.long option:selected').val(),
 	        ogcdVo={'level':level,'ogcd':ogcd,'pageNo':Nopage,'pageSize':10} 
 	        getOrgnlist(ogcdVo); 
 	     }
    });
