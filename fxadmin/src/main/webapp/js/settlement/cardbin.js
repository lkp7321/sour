...
require(['jquery','mmGrid','niceScroll','wdatePicker','dialog'],function($,mmGrid,niceScroll,wdatePicker,dialog){
	...
	//列参数;
    var cols = [
        { title:'卡bin', name:'cbin' ,width:120, align:'left' },
  ...
  //封装点击选择 请求机构
    ...
    //点击添加按钮；
	$('.add').click(function(){
		dialog.discountAdd('cardbin','卡bin优惠','保存');
	...
	$('.modify').click(function(){
		...
		if(a==0){
			dialog.choicedata('请选择一条数据','cardbin');
		}else if(a==1){
			dialog.discountAdd('cardbin','卡bin优惠','修改');
			$('.agentnum',parent.document).val( $('.agentnum').val());
		    $('.box tr').each(function(i,v){
				if( $(v).find('input[type="checkbox"]').prop('checked') ){
				    $('.channel',parent.document).val($(v).find('td').eq(1).find('span').text()).attr('disabled','disabled');
			...
			dialog.discountAdd('cardbin','机构优惠修改','修改');
			$('.agentnum',parent.document).val( $('.agentnum').val());
		}
		$('.buy',parent.document).blur(function(){
			 if($(this).val()==''||$(this).val()==undefined){
	...
	$('body',parent.document).on('click','.preserve',function(){
	    var ogcd=$('.agentnum').val(),
	        card=$('.channel',parent.document).val(),
	        ...
	        vo={'userKey':userkey,'ogcd':ogcd,'cbin':card,'byds':byds,'slds':slds,'stdt':beginDate,'eddt':endDate},arr=[];
	  ...
	  //判断结束日期不能为空
	    ...
	    if($(this).text()=="保存"){
		   if(num>=4){
	    	$.ajax({
				...
				success:function(data){
					 if(data.code==00){
					     ...
					     getOrgnlist(ogcdVo); 
						 renpage();
					 	 dialog.choicedata(data.codeMessage,'cardbin');
					 }else if(data.code==01){
						//异常
						 dialog.choicedata(data.codeMessage,'cardbin');
					}
		    ...
		    $('.box tr').each(function(i,v){
				if( $(v).find('input[type="checkbox"]').prop('checked') ){
				  arr.push({
					  'ogcd':$('.agentnum').val(),
					  'cbin':$(v).find('td').eq(1).find('span').text(),
		   ...
		   vo={'userkey':userkey,'ogcd':ogcd,'byds':byds,'slds':slds,'stdt':beginDate,'eddt':endDate,'chlist':arr};
		 if(num>=4){
		   $.ajax({
				url:'/fx/updateRuleByCards.do',
				type:'post',
				contentType:'application/json',
				data:JSON.stringify(vo),
				async:true,
	...
	//点击请选择一条数据的确定按钮；
	$('body',parent.document).on('click','.cardbin .sure',function(){
		  $(this).closest('.zhezhao').remove();
	});
	//关闭弹窗
	...
	function getOrgnlist(obj){
		$.ajax({
    		url:'/fx/getCardFavRuleByOgcd.do',
    		type:'post',
    		contentType:'application/json',
    		data:JSON.stringify(obj),
    		async:false,
    		  ...
    		  laypage.render({
    		    ...
    		    ,jump: function(obj,first){
    		    	if(!first){ //点击跳页触发函数自身，并传递当前页：obj.curr
    		    		getOrgnlist({'ogcd':sessionStorage.obj,'pageNo':obj.curr,'pageSize':$('.layui-laypage-limits select option:selected').attr('value')*1});
    		    	}	
    		    }
    		  });
    	});
