...
require(['jquery','mmGrid','niceScroll','dialog','WdatePicker'],function($,mmGrid,niceScroll,dialog,WdatePicker){
	...
	if(text=="成交流水监控"){
	    ...
	    var cols = [
	        ...
	        { title:'卖金额', name:'slam' ,width:80, align:'right'},
	        { title:'买金额', name:'byam' ,width:80, align:'right'},
	        { title:'折美元金额', name:'usam' ,width:80, align:'right'},
	        { title:'成交价格', name:'expc' ,width:80, align:'right'},
	    ...
	    var cols = [
	        ...
	        { title:'买金额', name:'bamt' ,width:80, align:'right'},
	        { title:'卖金额', name:'samt' ,width:80, align:'right'},
	        { title:'盈亏金额', name:'ykam' ,width:80, align:'right'},
	        { title:'交易状态', name:'stat' ,width:80, align:'center'}
    ...
    gettodayTranCount({'userKey':userkey,"btime":"","etime":""})
    $('.chaun').click(function(){
    	clearInterval(sT);
    	var ti=parseInt( $('.refrof').val() );
    	if( ti==''||!/^\d+$/.test( ti) ){
    	}else{
    		var sT=setInterval(function(){
    			var btime=$('#d12').val(),
	        	    etime=$('#d13').val();
	        	gettodayTranCount({'userKey':userkey,"btime":btime,"etime":etime});
        	},ti*1000);
    	}
     });
 //封装请求列表
