...
require(['jquery','mmGrid','niceScroll','dialog','wdatepicker'],function($,mmGrid,niceScroll,dialog,wdatepicker){
    ...
    var usnm=sessionStorage.getItem('usnm'),
		...
		userdata,chanpin,jiaoyandata,diandata,stoppai,productorigin,celue,ganyu,quanjuTime=0,listnumtotal;
    //判断数字的正则；
    var numreg=/^\d{1,4}$/,
    ...
    if(product=='P999'){
     ...
     if(product=="P007"){
    	 $('.effect').html('<button class="takeeffect pub_button">生效</button>');
     }
    //列参数;
    var cols = [
    		  ...
    		  laypage.render({
    		    ...
    		    ,jump: function(obj,first){
    		    	if(!first){ //点击跳页触发函数自身，并传递当前页：obj.curr
    		    		if(product=="P999"){
    		 	    		rendlist( {'url':'selectPdtRList.do','obj':{'userKey':userKey,'prod':prod,'pageNo':obj.curr,'pageSize':$('.layui-laypage-limits select option:selected').attr('value')*1 } });
    		 	    	}else{
    		 	    		rendlist( {'url':'selectPdtRList.do','obj':{'userKey':userKey,'pageNo':obj.curr,'pageSize':$('.layui-laypage-limits select option:selected').attr('value')*1} });
    		 	    	}
    		    	}	
    		    }
    		  });
						...
						if( userdata==1){                                            //判断展示对应的弹出框；
				           ...
				           // checkfn({'url':'ChkParaInit.do','data':{'userKey':userKey,'exnm':exnm },'obj':'quoteparameter','product':product});
							 ...
							 if(product=="P007"){
								 ...
								 dialog.priceModify("quoteparameter",0); 
							 }
