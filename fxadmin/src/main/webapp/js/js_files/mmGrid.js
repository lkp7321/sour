...
!function($){
    ...
    MMGrid.prototype = {
        ...
        , _populate: function(items){
            ...
            if(items && items.length !== 0 && opts.cols){
                ...
                tbodyHtmls.push('<tbody>');
          		 tbodyHtmls.push(this.$head.html());
        ...
        , deselect: function(args){
            ...
            var $head = this.$head;
            if(typeof args === 'number'){
                $body.find('tr').eq(args+1).removeClass('selected');
                /*if(opts.checkCol){
                    $body.find('tr').eq(args+1).find('td .mmg-check').prop('checked','');
                }*/
            }else if(typeof args === 'function'){
                $.each($body.find('tr'), function(index){
                    if(args($.data(this, 'item'), index)){
