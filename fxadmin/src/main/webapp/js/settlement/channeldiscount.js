...
require(['jquery','mmGrid','niceScroll','wdatePicker','dialog'],function($,mmGrid,niceScroll,wdatePicker,dialog){
    ...
    var ogcd=sessionStorage.obj,
        chnl=$('.long option:selected').val(),
        ogcdVo={'chnl':chnl,'ogcd':ogcd,'pageNo':1,'pageSize':10} 
    getOrgnlist(ogcdVo);
    renpage();
    //根据客户等级的改变重新请求列表
  ...
  //封装点击选择 请求机构
    ...
    //点击添加按钮；
	$('.add').click(function(){
		dialog.discountAdd('channeldiscount','渠道优惠','保存');
		$('.agentnum',parent.document).val($('.agentnum').val());
		$('.channel',parent.document).val($('.long option:selected').text());
		$('.channel',parent.document).attr('chnl',$('.long option:selected').val());
	...
	$('.modify').click(function(){
		...
		if(a==0){
			dialog.choicedata('请选择一条数据','channeldiscount');
		}else if(a==1){
			dialog.discountAdd('channeldiscount','渠道优惠','修改');
			$('.agentnum',parent.document).val( $('.agentnum').val());
			$('.channel',parent.document).val($('.long option:selected').text());
			$('.channel',parent.document).attr('chnl',$('.long option:selected').val());
	...
	$('body',parent.document).on('click','.preserve',function(){
	    var ogcd=$('.agentnum').val(),
	        chnl=$('.channel',parent.document).attr('chnl'),
	        ...
	        vo={'userKey':userkey,'ogcd':ogcd,'chnl':chnl,'currency':currency,'byds':byds,'slds':slds,'beginDate':beginDate,'endDate':endDate},arr=[];
	  ...
	  //判断结束日期不能为空
	    ...
	    if($(this).text()=="保存"){
		  if(num>=4){ 
	    	$.ajax({
				...
				success:function(data){
					 if(data.code==00){
					     ...
					     getOrgnlist(ogcdVo); 
						 renpage();
					 	 dialog.choicedata(data.codeMessage,'channeldiscount');
					 }else if(data.code==01){
						//异常
						 dialog.choicedata(data.codeMessage,'channeldiscount');
					}
		   ...
		   vo={'userKey':userkey,'ogcd':ogcd,'chnl':chnl,'byds':byds,'slds':slds,'beginDate':beginDate,'endDate':endDate,'chlist':arr};
		 if(num>=4){
		   $.ajax({
				url:'/fx/updateRuleByChannels.do',
				type:'post',
				contentType:'application/json',
				data:JSON.stringify(vo),
				async:true,
	...
	$('body',parent.document).on('click','.discountpub .confirm',function(){
		   var ogcd=$('.agentnum').val(),
		       chnl=$('.long option:selected').val(),arr=[],vo;
		   $('.box tr').each(function(i,v){
				if( $(v).find('input[type="checkbox"]').prop('checked') ){
				  arr.push({
					  'exnm':$(v).find('td').eq(1).find('span').text()
		  ...
		  vo={'userkey':userkey,'ogcd':ogcd,'chnl':chnl,'chlist':arr};
		   $.ajax({
				url:'/fx/deleteChannelFavrRule.do',
				type:'post',
				contentType:'application/json',
				data:JSON.stringify(vo),
				async:true,
				success:function(data){
					 if(data.code==00){
						 ...
						 ogcd=$('.agentnum').val(),
					     ogcdVo={'ogcd':ogcd,'chnl':chnl,'pageNo':1,'pageSize':10} 
					     getOrgnlist(ogcdVo); 
						 renpage();
					 	 dialog.choicedata(data.codeMessage,'channeldiscount');
					 }else if(data.code==01){
	...
	//点击请选择一条数据的确定按钮；
	$('body',parent.document).on('click','.channeldiscount .sure',function(){
		  $(this).closest('.zhezhao').remove();
	});
	//关闭弹窗
	...
	function getOrgnlist(obj){
		$.ajax({
    		url:'/fx/getChannelFavRuleByOgcd.do',
    		type:'post',
    		contentType:'application/json',
    		data:JSON.stringify(obj),
    		async:false,
    ...
    /*$('.boxtop').on('click','.page .first',function(){
    	var Nopage=parseInt($('.Nopage').text());
 	    if(Nopage>1){
 	    	...
 	    	ogcd=$('.agentnum').val(),
 	        chnl=$('.long option:selected').val(),
 	        ogcdVo={'chnl':chnl,'ogcd':ogcd,'pageNo':Nopage,'pageSize':10} 
 	        getOrgnlist(ogcdVo); 
 	     }
