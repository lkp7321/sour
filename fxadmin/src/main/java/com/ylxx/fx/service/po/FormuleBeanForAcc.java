package com.ylxx.fx.service.po;

public class FormuleBeanForAcc {
	public String prcd = "";
	public String cknm = "";
	public String exnm = "";
	public String excd = "";
	public double lamt = 0;
	public double ramt = 0;
	public String cbhl = "0";
	public double fdsy = 0;
	public double price = 0;
	public double pricer = 0;
	private String udfg = "";
	private double ppyk = 0;
	private double zcyk = 0;
	private double sgyk;
	private double tryk;
	private double toyk;

	private double sumfdsy=0;

	public String getPrcd() {
		return prcd;
	}

	public void setPrcd(String prcd) {
		this.prcd = prcd;
	}

	public String getCknm() {
		return cknm;
	}

	public void setCknm(String cknm) {
		this.cknm = cknm;
	}

	public String getExnm() {
		return exnm;
	}

	public void setExnm(String exnm) {
		this.exnm = exnm;
	}

	public String getExcd() {
		return excd;
	}

	public void setExcd(String excd) {
		this.excd = excd;
	}

	public double getLamt() {
		return lamt;
	}

	public void setLamt(double lamt) {
		this.lamt = lamt;
	}

	public double getRamt() {
		return ramt;
	}

	public void setRamt(double ramt) {
		this.ramt = ramt;
	}

	

	public String getCbhl() {
		return cbhl;
	}

	public void setCbhl(String cbhl) {
		this.cbhl = cbhl;
	}

	public double getFdsy() {
		return fdsy;
	}

	public void setFdsy(double fdsy) {
		this.fdsy = fdsy;
	}

	public double getPrice() {
		return price;
	}

	public void setPrice(double price) {
		this.price = price;
	}

	public double getPricer() {
		return pricer;
	}

	public void setPricer(double pricer) {
		this.pricer = pricer;
	}

	public String getUdfg() {
		return udfg;
	}

	public void setUdfg(String udfg) {
		this.udfg = udfg;
	}

	public double getPpyk() {
		return ppyk;
	}

	public void setPpyk(double ppyk) {
		this.ppyk = ppyk;
	}

	public double getZcyk() {
		return zcyk;
	}

	public void setZcyk(double zcyk) {
		this.zcyk = zcyk;
	}

	public double getSgyk() {
		return sgyk;
	}

	public void setSgyk(double sgyk) {
		this.sgyk = sgyk;
	}

	public double getTryk() {
		return tryk;
	}

	public void setTryk(double tryk) {
		this.tryk = tryk;
	}

	public double getToyk() {
		return toyk;
	}

	public void setToyk(double toyk) {
		this.toyk = toyk;
	}

	public double getSumfdsy() {
		return sumfdsy;
	}

	public void setSumfdsy(double sumfdsy) {
		this.sumfdsy = sumfdsy;
	}
	
	
}
