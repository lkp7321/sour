...
//	if(!listdata||!product){
//		dialog.choicedata('您还未登录,请先去登录<br/>'+setinnum+'秒后自动跳转&nbsp;<a href="http://127.0.0.1:8080/fx/" tit="立即跳转">立即跳转</a>','index');
//	    
...
//				location.href="http://127.0.0.1:8080/fx/";
//			}else{
//				$('.index p').html('您还未登录,请先去登录<br/>'+setinnum+'秒后自动跳转&nbsp;<a href="http://127.0.0.1:8080/fx/" tit="立即跳转">立即跳转</a>');
//			}
//		},1000);
//	}else{
		$('.username').text( sessionStorage.getItem('usnm') );
		switch(product){
			case 'P001':product='个人外汇买卖平台';break;
			case 'P002':product='纸黄金交易管理平台';break;
			case 'P003':product='积存金管理平台';break;
			case 'P004':product='个人结售汇';break;
			case 'P007':product='账户交易';break;
			case 'P009':product='保证金前置';break;
			case 'P998':product='RV前置';break;
			case 'P999':product='报价平台';break;
			case 'JH01':product='包商银行';break;
		}
		$('.topright h6').text( product);
		...
		if(listdata!=''){
			...
			$('.siderbars>li').each(function(){
				if( $(this).children('a').text()=='平盘管理' ){	//平盘管理;
			...
			$('.list-group-ul>li').each(function(){
				...
				if( $(this).children('a').text()=='积存金点差设置' ){	//隐藏特殊点差设置；
					$(this).closest('li').hide();
				}
	...
	//}
		...
		$('.list-group-ul li:not(".list-group-thr")').click(function(e){
			...
			var t=$(this).find('a').text(),
				tt=$(this).parent('ul').siblings('a').text(),fold,
				ttt=$(this).closest('.list-group-ul').siblings('a').text();
			    chan=$('.topright h6',parent.document).text();
				//$('#ifra').attr('src',checklist(t,tt,ttt));
			    var t=$(this).find('a').attr('ur');
				//更改右侧列表；
			    $('#ifra').attr('src',t);
		});
		$('.list-group-ul li:not(".list-group-thr")').hover(function(e){
			var evt=e||window.event;
			if(evt.stopPropagation ){
				evt.stopPropagation();
			}else{
			...
			$('.list-group-thr li').siblings().removeClass('liHov');
			$(this).siblings().removeClass('liHov');
			$(this).addClass('liHov');
		});
		//点击三级中的li;
		$('.list-group-thr li').hover(function(e){
			var evt=e||window.event;
			if(evt.stopPropagation ){
				evt.stopPropagation();
			}else{
			...
			$('.list-group-ul li').siblings().removeClass('liHov');
			$(this).siblings().removeClass('liHov');
			$(this).addClass('liHov');
		});
		//滑出li;
		
		$('.siderbars li').hover(function(e){
			var evt=e||window.event;
			if(evt.stopPropagation ){
				evt.stopPropagation();
			}else{
		...
		/* $('.list-group-ul li').click(function(){
			 $(this).addClass('liHov')
			 $('.list-group-ul li').removeClass('liHov');
		 })*/
